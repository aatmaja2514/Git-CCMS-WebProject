...
//{
    ...
    else if(flag==0) 
    {
            ...
            if (rs1.next())
            {
               ...
               /* PreparedStatement update_status1 = con.prepareStatement("UPDATE member SET Challenge_Id = ? WHERE Status = 'MS' AND Status = 'T' AND Club_Id = ?");
                update_status1.setString(1, rs6.getString(1));
                update_status1.setString(2, session_Club_Id);
                update_status1.executeUpdate();*/
                        ...
                        while(rs7.next())
                        {
                            ...
                            for ( i = 0; i < Array_chids.length; i++) 
                            {
                                ...
                                if(rs11.getString(1).equals("Finished"))
                                {
                                    ...
                                    if(rs7.getString(3).equals("MS") && (strbuffer.toString().equals(" ") || strbuffer.toString().equals("")))
                                    {
                                    	Statement update_ch_id = con.createStatement();
                                    	update_ch_id.executeUpdate("UPDATE member SET Status = 'M', Challenge_Id = ' ' WHERE Member_Id = '" + rs7.getString(2) + "'");
                                    }
                                    else
                                    {
                                    	Statement update_ch_id = con.createStatement();
                                    	update_ch_id.executeUpdate("UPDATE member SET Challenge_Id = '" + strbuffer.toString() + "' WHERE Member_Id = '" + rs7.getString(2) + "'");
                                    }
                                }
                            }
                        }
                ...
                if (rs3.next()) 
                {
                    ...
                    if (rs4.next()) 
                    {
                        ...
                        if (rs5.next())
                        {
                            ...
                            String desc1 = "<a href=''>Edit your team before accepting.</a>" + ch_club_name + " challenged you. ";
