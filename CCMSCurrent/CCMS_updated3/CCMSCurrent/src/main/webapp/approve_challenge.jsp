	      		...
	      		ResultSet rs3 = st3.executeQuery("SELECT Club_Name,Club_Id FROM clubs WHERE Club_Id = (SELECT opp_club FROM challenge WHERE Challenge_Id = '"+ accept_challengeid +"' AND Status ='Approved')");
  	      		...
  	      		if(rs.next() && rs3.next())
  	      		{
  	               ...
  	               /* Statement stg7 = con.createStatement();  
  	                ResultSet rs7 = stg7.executeQuery("SELECT Challenge_Id, Member_Id FROM member WHERE (Status = 'T' OR Status = 'MS' OR Status = 'TS') AND (Club_Id = '" + rs.getString(2) + "' OR Club_Id = '" + rs3.getString(2) + "')");
  		            while(rs7.next())
  		            {
  		            	...
  		            	if(str_chids.equals(" "))
  		            	{
	                      	...
	                      	update_ch_id.executeUpdate("UPDATE member SET Challenge_Id = '" + accept_challengeid + "' WHERE Member_Id = '" + rs7.getString(2) + "'");
  		            	}
  		            	else
  		            	{
	  		                String added_chid = str_chids + " " + accept_challengeid;
	  		                
	  		              	Statement update_ch_id = con.createStatement();
	                      	update_ch_id.executeUpdate("UPDATE member SET Challenge_Id = '" + added_chid + "' WHERE Member_Id = '" + rs7.getString(2) + "'");
  		            	}
  		            }*/
